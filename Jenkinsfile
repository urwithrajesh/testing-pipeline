...
stage 'Download'
    node(jenkins) {
        echo 'Building.......'
        notifyBuildSlack('Starting Dev JOB','chatops')
...
stage 'SonarQube'
    node(jenkins) {
        ...
        withSonarQubeEnv('SonarQube') {
          sh ' /var/lib/jenkins/tools/hudson.plugins.sonar.SonarRunnerInstallation/SonarQube/bin/sonar-scanner -Dsonar.projectBaseDir=/var/lib/jenkins/workspace/CICD/CICD-Demo-Dev'
            }
    }
...
stage 'Deploy'
    ...
    sh '''
        ...
        '''
     notifyDeploySlack('Dev Job is Finished','chatops')
    }
